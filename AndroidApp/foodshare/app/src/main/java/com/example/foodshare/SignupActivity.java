...
public class SignupActivity extends BaseActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        checkBox_user = findViewById(R.id.checkBox_user);
        checkBox_res = findViewById(R.id.checkBox_res);
    ...
    private void onAuthSuccess(FirebaseUser user) {
        ...
        Boolean resCheck = checkBox_res.isChecked();
        Boolean userCkeck = checkBox_user.isChecked();
        if(resCheck) {
            // Write new user to database
            writeNewUser(user.getUid(), username, user.getEmail(), address, phone, intro, 5, "", 0, "restaurant");
        }
        if(userCkeck) {
            // Write new user to database
            writeNewUser(user.getUid(), username, user.getEmail(), address, phone, intro, 5, "", 0, "user");
        }
    ...
    private void writeNewUser(String userId, String name, String email, String address, String phone, String intro, double rate, String avatar, int totalReview, String category) {
        User user = new User(name, email, address, phone, intro, rate, avatar, totalReview, category);
        mDatabase.child("users").child(userId).setValue(user);
    }

    private  void onClickCheck() {
        checkBox_user.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                boolean checked = ((CheckBox) v).isChecked();
                // Check which checkbox was clicked
                if (checked){
                    checkBox_res.setEnabled(false);
                }
                else{
                    checkBox_res.setEnabled(true);
                }
            }
        });
        checkBox_res.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                boolean checked = ((CheckBox) v).isChecked();
                // Check which checkbox was clicked
                if (checked){
                    checkBox_user.setEnabled(false);
                }
                else{
